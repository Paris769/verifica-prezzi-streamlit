...
with st.expander("Impostazioni colonne (default secondo istruzioni)"):
...
if st.button("Esegui verifica", type="primary"):
    ...
    try:
        ...
        st.download_button("Scarica CSV", data=csv_bytes, file_name="report_verifica_prezzi.csv", mime="text/csv")

        xls_bytes = to_excel_bytes(report)
        st.download_button("Scarica Excel", data=xls_bytes, file_name="report_verifica_prezzi.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
